...
@testable import XGoalPicker; import XEntities; import XCoordinator;
...
final class XGoalPickerServiceInterfaceMock: XGoalPickerServiceInterface {

    //MARK: - getAvailableGoals

    var getAvailableGoalsThrowableError: Error?
    var getAvailableGoalsCallsCount = 0
    var getAvailableGoalsCalled: Bool {
        return getAvailableGoalsCallsCount > 0
    }
    var getAvailableGoalsReturnValue: [GoalEntity]!
    var getAvailableGoalsClosure: (() throws -> [GoalEntity])?

    func getAvailableGoals() async throws -> [GoalEntity] {
        if let error = getAvailableGoalsThrowableError {
            throw error
        }
        getAvailableGoalsCallsCount += 1
        return try getAvailableGoalsClosure.map({ try $0() }) ?? getAvailableGoalsReturnValue
    }
