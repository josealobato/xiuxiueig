...
@testable import XQueueCollection; import XEntities; import XCoordinator;
...
final class XQueueCollectionServiceInterfaceMock: XQueueCollectionServiceInterface {

    //MARK: - queuedLectures

    var queuedLecturesThrowableError: Error?
    var queuedLecturesCallsCount = 0
    var queuedLecturesCalled: Bool {
        return queuedLecturesCallsCount > 0
    }
    var queuedLecturesReturnValue: [LectureEntity]!
    var queuedLecturesClosure: (() throws -> [LectureEntity])?

    func queuedLectures() async throws -> [LectureEntity] {
        if let error = queuedLecturesThrowableError {
            throw error
        }
        queuedLecturesCallsCount += 1
        return try queuedLecturesClosure.map({ try $0() }) ?? queuedLecturesReturnValue
    }
